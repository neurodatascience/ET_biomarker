...
COPY docker_/neurodebian.gpg /usr/local/etc/neurodebian.gpg
...
# Precaching atlases
COPY docker_/vincent_env-setup.cfg vincent_env-setup.cfg
COPY docker_/fetch_templates.py fetch_templates.py

RUN pip install --no-cache-dir "$( grep templateflow vincent_env-setup.cfg | xargs )" && \
    python fetch_templates.py && \
    rm vincent_env-setup.cfg fetch_templates.py && \
    find $HOME/.cache/templateflow -type d -exec chmod go=u {} + && \
    find $HOME/.cache/templateflow -type f -exec chmod go=u {} +
#####
# Installing my owntool
#COPY . /src/fmriprep 
#ARG VERSION
